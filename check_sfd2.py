...
try:
    ...
    response = gtw.request("22F190", tries=2, timeout=5)
    if response:
    ...
    response = gtw.request("22F187", tries=2, timeout=5)
    if response:
        print("Request OK")
    ...
    print("Sending request for ECU TYPE...")
    response = gtw.request("22F19E", tries=2, timeout=5)
    if response:
            print("Request OK")
            # Decode Supplier Number, ignoring the first 3 bytes
            ECU_TYPE = bytes.fromhex(response[6:]).decode("utf-8")
            print(f"ECU_TYPE: {ECU_TYPE}")

            if "UNECE" in ECU_TYPE:
                print(colorama.Fore.RED + "The vehicle had SFD2 Version 2 gateway!")
            else:
                print(colorama.Fore.GREEN + "The vehicle had no SFD2")
    else:
            print("Request failed: ECU_TYPE NOT FOUND")
#SW_versie request=================================================

    print("Sending request for SW Version...")
    response = gtw.request("22F189", tries=2, timeout=5)
